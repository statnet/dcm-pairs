...
#' @param s.num number of susceptible males in the population
#' @param i.num number of chronically infected males in the population. 
#' @param s.num.g2 number of susceptible females in the population
#' @param i.num.g2 number of chronically infected females in the population. 
#' @param p.num number of primary stage infected  males in the population*
#' @param p.num.g2 number of primary stage infected fe males in the population*
...
#' 
#' Single male compartments:s1.num, p1.num, i1.num,
#' Single female compartments: s1.num.g2, p1.num.g2, i1.num.g2,
...
#' @param concur.rate rate of acts outside pair for males in pairs 
#' @param concur.rate.g2 rate of acts outside pair for females in pairs 
...
#' @param b.rate or bs.rate birth/immigration rate for susceptible males into the population.
#' @param b.rate.g2 or bs.rate.g2 birth/immigration rate for susceptible females into the population.
#' @param bp.rate same for primary infected males into the population. (opt)
#' @param bp.rate.g2 same for primary infected females into the population. (opt)
#' @param bi.rate same for chronically infected males into the population.
#' @param bi.rate.g2 same for chronically infected females into the population.
#' @param ds.rate mortality rate for susceptible males
#' @param ds.rate.g2 mortality rate for susceptible females
#' @param dp.rate mortality rate for males during primary infection
#' @param dp.rate.g2 mortality rate for females during primary infection
#' @param di.rate mortality rate for males during chronic infection
#' @param di.rate.g2 mortality rate for females during chronic infection
#'
#'
#' @details 
...
init.pf <- function(s.num, 
	...
	fp.num <- 0
      ...
      s1.num = msingles.num * (ms.num/m.num)
      p1.num = msingles.num * (mp.num/m.num)
      i1.num = msingles.num * (mi.num/m.num)
      s1.num.g2 = fsingles.num * (fs.num/f.num)
      p1.num.g2 = fsingles.num * (fp.num/f.num)
      i1.num.g2 = fsingles.num * (fi.num/f.num)
...
pf_model <- function(t, t0, parms) {
  ...
  with(as.list(c(t0, parms)), {
	...
	verbose=FALSE
      ...
      msingles.num = s1.num + p1.num + i1.num
      fsingles.num = s1.num.g2 + p1.num.g2 + i1.num.g2
      singles.num = msingles.num + fsingles.num
      
      s2.num = p_ss.num + p_smpf.num + p_smif.num
      p2.num = p_pp.num + p_pmif.num + p_pmsf.num
      i2.num = p_ii.num + p_impf.num + p_imsf.num

      s2.num.g2 = p_ss.num + p_pmsf.num + p_imsf.num
      p2.num.g2 = p_pp.num + p_smpf.num + p_impf.num
      i2.num.g2 = p_ii.num + p_pmif.num + p_smif.num
      ...
      # contact rate for single males (excludes contacts w/ paired females)
      if (balance == "g1") {
	ct.g1 <- act.rate
	ct.g2 <- ct.g1 * msingles.num / fsingles.num

	pair.m <- formation.rate
	pair.f <- pair.m * msingles.num / fsingles.num
      }
      if (balance == "g2") {
	ct.g2 <- act.rate.g2
	ct.g1 <- ct.g2 * fsingles.num / msingles.num

	pair.f <- formation.rate.g2
	...
	bs.rate <- 0
      ...
      if (! exists("bs.rate.g2")) {
	bs.rate.g2 <- bs.rate
      }
      brs.m <- bs.rate*num.g1
      brs.f <- bs.rate.g2*num.g2
      print(c(brs.m,brs.f))

      if (! exists ("bp.rate")) {
	bp.rate <- 0
      }
      brp.m <- bp.rate*num.g1

      if (! exists ("bp.rate.g2")) {
	bp.rate.g2 <- bp.rate
      }
      brp.f <- bp.rate.g2*num.g2

      if (! exists ("bi.rate")) {
	bi.rate <- 0
      }
      bri.m <- bi.rate*num.g1

      if (! exists ("bi.rate.g2")) {
	bi.rate.g2 <- bi.rate
      }
      bri.f <- bi.rate.g2*num.g2
	...
	di.rate.g2 <- di.rate
      ...
      dem_dSm <- brs.m - ds.rate*s1.num +
	      ds.rate.g2*p_ss.num + dp.rate.g2*p_smpf.num  + di.rate.g2*p_smif.num 
      dem_dSf <- brs.f - ds.rate.g2*s1.num.g2 +
	      ds.rate*p_ss.num + dp.rate*p_pmsf.num  + di.rate*p_imsf.num 

      dem_dPm <- brp.m -dp.rate*p1.num +
	      dp.rate.g2*p_pp.num + ds.rate.g2*p_pmsf.num  + di.rate.g2*p_pmif.num 
      dem_dPf <- brp.f -dp.rate.g2*p1.num.g2 +
	      dp.rate*p_pp.num + ds.rate*p_smpf.num  + di.rate*p_impf.num 

      dem_dIm <- bri.m -di.rate*i1.num +
	      di.rate.g2*p_ii.num + ds.rate.g2*p_imsf.num  + dp.rate.g2*p_impf.num 
      dem_dIf <- bri.f -di.rate.g2*i1.num.g2 +
	      di.rate*p_ii.num + ds.rate*p_smif.num  + dp.rate*p_pmif.num 
	...
	print(c(dem_dSm,dem_dSf,dem_dPm,dem_dPf,dem_dIm,dem_dIf))
      ...
      dem_dSmPf <- -dp.rate.g2*p_smpf.num - ds.rate*p_smpf.num
      dem_dSmIf <- -di.rate.g2*p_smif.num - ds.rate*p_smif.num
      dem_dPmSf <- -ds.rate.g2*p_pmsf.num - dp.rate*p_pmsf.num
      dem_dPP   <- -dp.rate*p_pp.num - dp.rate.g2*p_pp.num
      dem_dPmIf <- -dp.rate*p_pmif.num - di.rate.g2*p_pmif.num
      dem_dImSf <- -di.rate*p_imsf.num - ds.rate.g2*p_imsf.num
      dem_dImPf <- -di.rate*p_impf.num - dp.rate.g2*p_impf.num
	...
	exit(1)
      ...
      pf_dSS <- pair.m * s1.num*s1.num.g2/(fsingles.num) - separate.rate * p_ss.num
      pf_dSmPf <- pair.m * s1.num*p1.num.g2/(fsingles.num) - separate.rate * p_smpf.num
      pf_dSmIf <- pair.m * s1.num*i1.num.g2/(fsingles.num) - separate.rate * p_smif.num
      pf_dPmSf <- pair.m * p1.num*s1.num.g2/(fsingles.num) - separate.rate * p_pmsf.num
      pf_dPP   <- pair.m * p1.num*p1.num.g2/(fsingles.num) - separate.rate * p_pp.num
      pf_dPmIf <- pair.m * p1.num*i1.num.g2/(fsingles.num) - separate.rate * p_pmif.num
      pf_dImSf <- pair.m * i1.num*s1.num.g2/(fsingles.num) - separate.rate * p_imsf.num
      pf_dImPf <- pair.m * i1.num*p1.num.g2/(fsingles.num) - separate.rate * p_impf.num
      pf_dII   <- pair.m * i1.num*i1.num.g2/(fsingles.num) - separate.rate * p_ii.num
	...
	exit(1)
      ...
      female1S.rate = trans.rate.g2*((ct.g2*i1.num/msingles.num)+pfactor*(ct.g2*p1.num/msingles.num))
      if (p_pairs.num > 0)
	female1S.rate = female1S.rate + trans.rate.g2*((concur.rate*(p_imsf.num+p_impf.num+p_ii.num)/p_pairs.num)+pfactor*(concur.rate*(p_pmif.num+p_pmsf.num+p_pp.num)/p_pairs.num))
      male1S.rate = trans.rate*(((ct.g1*i1.num.g2/fsingles.num))+pfactor*(ct.g1*p1.num.g2/fsingles.num))
      if (p_pairs.num > 0)
	male1S.rate = male1S.rate + trans.rate*((concur.rate.g2*(p_smif.num+p_pmif.num+p_ii.num)/p_pairs.num)+pfactor*(concur.rate.g2*(p_smpf.num+p_impf.num+p_pp.num)/p_pairs.num))

      inf_dSm <- -male1S.rate*s1.num
      inf_dSf <- -female1S.rate*s1.num.g2
      if (pi.rate > 0) {
	inf_dPm <- male1S.rate*s1.num- pi.rate*p1.num
	inf_dPf <- female1S.rate*s1.num.g2- pi.rate*p1.num.g2
	inf_dIm <- pi.rate*p1.num
	inf_dIf <- pi.rate*p1.num.g2
      } else {
	inf_dPm <- 0
	inf_dPf <- 0
	inf_dIm <- male1S.rate*s1.num
	inf_dIf <- female1S.rate*s1.num.g2
      }
	...
	exit(1)
      ...
      fSStoSP.rate = trans.rate.g2*concur.rate.g2*
	      (i1.num/msingles.num + pfactor*p1.num/msingles.num)
      mSStoPS.rate = trans.rate*concur.rate*
	      (i1.num.g2/fsingles.num + pfactor*p1.num.g2/fsingles.num)

      # rates when infection can comes from outside or within pair
      fSPtoPP.rate = fSStoSP.rate+trans.rate.g2*p_act.rate*pfactor
      mSPtoPP.rate = mSStoPS.rate+trans.rate*p_act.rate*pfactor

      fSItoPI.rate = fSStoSP.rate+trans.rate.g2*p_act.rate
      mSItoPI.rate = mSStoPS.rate+trans.rate*p_act.rate
	...
	msp.flow <- male1S.rate*s1.num + SS2PS.flow+SP2PP.flow+SI2PI.flow
	fsp.flow <- female1S.rate*s1.num.g2 + SS2SP.flow+PS2PP.flow+IS2IP.flow
	mpi.flow <- pi.rate*p1.num + PS2IS.flow+PP2IP.flow+PI2II.flow
	fpi.flow <- pi.rate*p1.num.g2 + SP2SI.flow+PP2PI.flow+IP2II.flow
      } else {
	...
	msp.flow <- male1S.rate*s1.num + SS2IS.flow+SI2II.flow
	fsp.flow <- female1S.rate*s1.num.g2 + SS2SI.flow+IS2II.flow
	mpi.flow <- 0
	fpi.flow <- 0
      }
	...
	print(c(dSS,dSmPf,dSmIf,dPmSf,dPP,
      ...
      list(c(dSm, dPm, dIm, dSf, dPf, dIf,dSS,dSmPf,dSmIf,dPmSf,dPP,dPmIf,dImSf,dImPf,dII), 
	s1.num = s1.num,
	p1.num = p1.num,
	i1.num = i1.num,
	s1.num.g2 = s1.num.g2,
